...
def write_common_general(write) -> None:
    ...
    lines = (
        ...
        "**.**.tcp.conn-*.semiCoupledEpsilonPathPrice.statistic-recording = true",
        "**.**.tcp.conn-*.semiCoupledEpsilonOpportunity.statistic-recording = true",
        "**.**.tcp.conn-*.semiCoupledEpsilonTargetShare.statistic-recording = true",
        "**.**.tcp.conn-*.semiCoupledEpsilonRateShare.statistic-recording = true",
        "**.**.tcp.conn-*.semiCoupledEpsilonResponsiveness.statistic-recording = true",
        "**.**.tcp.conn-*.semiCoupledEpsilonAiShare.statistic-recording = true",
        "**.**.tcp.conn-*.semiCoupledEpsilonAiRateBudget.statistic-recording = true",
        "**.**.tcp.conn-*.semiCoupledEpsilonAdjustedAi.statistic-recording = true",
...
def write_protocol_settings(write, protocol: str, settings: dict[str, str]) -> None:
    ...
    if is_mporb:
        ...
        write("**.eta = 0.95")
        if protocol in {"mporb_alpha", "mporb_delta", "mporb_epsilon"}:
