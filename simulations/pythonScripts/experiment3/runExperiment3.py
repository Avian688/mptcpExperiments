...
CONFIGS = [
    ...
    ("MpOrbTheta", "mporb_theta", "experiment3_mporb_theta.ini"),
]
DEFAULT_RUNS = 5
...
LOG_DIR = SIM_ROOT / "logs" / "experiment3"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run mptcpExperiments experiment 3.")
...
def main() -> int:
    ...
    try:
        if not args.skip_generate:
            run_checked([sys.executable, "generateExperiment3IniFiles.py"], SCRIPT_DIR, "Generating experiment 3 ini files")
        ...
        if args.clean:
            ...
            shutil.rmtree(SIM_ROOT / "plots" / "experiment3", ignore_errors=True)
        if enabled(1, args):
        ...
        if enabled(4, args):
            command = [sys.executable, str(SCRIPT_DIR / "plotExperiment3.py"), "--runs"]
            ...
            summary_command = [
                sys.executable,
                str(SCRIPT_DIR / "summarizeExperiment3.py"),
                "--runs",
            ]
