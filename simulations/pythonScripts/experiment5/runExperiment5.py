...
CONFIGS = [
    ("CubicUncoupled", "cubic", "experiment5_cubic.ini"),
    ("MpOrbUncoupled", "mporb", "experiment5_mporb.ini"),
    ("MpOrbSemiCoupledAlpha", "mporb_semicoupled_alpha", "experiment5_mporb_semicoupled_alpha.ini"),
    ("MpOrbOlia", "mporb_olia", "experiment5_mporb_olia.ini"),
    ("MpOrbSemiCoupledBeta", "mporb_semicoupled_beta", "experiment5_mporb_semicoupled_beta.ini"),
    ("MpOrbSemiCoupledEpsilon", "mporb_semicoupled_epsilon", "experiment5_mporb_semicoupled_epsilon.ini"),
    ("MpOrbSemiCoupledTheta", "mporb_semicoupled_theta", "experiment5_mporb_semicoupled_theta.ini"),
    ("LiaCoupled", "lia", "experiment5_lia.ini"),
    ("OliaCoupled", "olia", "experiment5_olia.ini"),
    ("BaliaCoupled", "balia", "experiment5_balia.ini"),
]
DEFAULT_CONFIGS = [
    "MpOrbUncoupled",
...
EXPERIMENT_DIR = SIM_ROOT / "experiments" / "experiment5"
RESULTS_DIR = EXPERIMENT_DIR / "results"
LOG_DIR = SIM_ROOT / "logs" / "experiment5"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run mptcpExperiments experiment 5.")
    ...
    parser.add_argument("--clean", action="store_true", help="Remove experiment5 results/csvs/plots before running.")
    ...
    if args.end_step == 4 and args.sim_time_limit not in (None, "150s"):
        parser.error("Phase plots require 150s; use --end-step 3 for a shortened diagnostic run")
    return args
...
def generate_inputs() -> None:
    run_checked(
        [sys.executable, "generateExperiment5IniFiles.py"],
        SCRIPT_DIR,
        "Generating experiment 5 ini files",
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.clean:
            ...
            shutil.rmtree(SIM_ROOT / "plots" / "experiment5", ignore_errors=True)
        ...
        if enabled(4, args):
            command = [
                sys.executable,
                str(SCRIPT_DIR / "plotExperiment5.py"),
                "--protocols",
